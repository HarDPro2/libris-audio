...
|:--|:-------|:------------|:----------|
| B1 | **Descarga offline + inventario** | **✅ HECHO.** `OfflineManager.kt` descarga audio+texto+timing por parte a `filesDir/offline/{bookId}/` con `meta.json`. Botón de descarga (con progreso) en el reproductor. Sección "📥 Descargas" en Ajustes: lista con tamaño por libro, borrar individual y borrar todo. Reproducción con **fallback a archivo local** en `buildMediaItem`/`loadPartText`/`loadTiming` (audio, texto y karaoke sin conexión). Descarga reanudable (salta partes ya bajadas). **Pulido futuro:** descargas en segundo plano con servicio dedicado + notificación (hoy corre mientras la app está abierta). | Alta |
...
| B3 | **Animación de fondo en el reproductor** | **✅ HECHO.** `AnimatedBackground` del tema ya presente en Reproductor, Biblioteca, Historial y Ajustes; se añadió también al Login para consistencia total. Car Mode se deja limpio a propósito (alto contraste para conducir). | Baja |
| B4 | **Versionado + auto-actualizador** | **✅ HECHO.** Versión desde `package.json` inyectada al build (`-PlibrisVersionName`/`-PlibrisVersionCode`), APK con nombre fijo `libris-audio.apk` en CI + landing, y auto-actualizador in-app (`UpdateManager.kt`: consulta GitHub Releases, compara con `BuildConfig.VERSION_NAME`, descarga e instala vía FileProvider). Para publicar: subir versión en `package.json` antes del push. | Media |
| B5 | **Sincronización en la nube (progreso + preferencias)** | Colección `user_state` en Appwrite (un doc por usuario con JSON), endpoints backend GET/PUT, y en la app restaurar al login + guardar al cambiar. Para continuar donde quedó en cualquier dispositivo/reinstalación. **✅ HECHO** (progreso, tema, voz, racha, favoritos). Falta: sincronizar también el perfil (nombre+avatar). | Alta |
| B6 | **Marcos elaborados animados por género (subtemas)** | **✅ HECHO.** `GenreFrameOverlay.kt`: capa animada en Canvas con "clima" propio por género (polvo dorado clásico, brasas medievales, pétalos romance, luciérnagas fantasía, lluvia noir, estrellas cósmicas, confeti comedia, humo guerra, wisps paranormal/espiritual, nodos científicos, tinta poesía). Se dibuja detrás del texto con alpha baja + toggle "🎇 FX" en la barra. | Media |
| B9 | **Asistente de voz (2 tiers + offline)** | Ver `PLAN_ASISTENTE_VOZ_Y_BILINGUE.md`. Free: comandos on-device ($0, offline) — parar/seguir/retroceder/ir a capítulo. Plus: lenguaje natural vía LLM con la API key propia del usuario (BYOK) + búsqueda semántica de capítulos. Base: control por sistema (Assistant/Auto/Bluetooth). | Alta |
| B10 | **App bilingüe (ES/EN)** | Ver `PLAN_ASISTENTE_VOZ_Y_BILINGUE.md`. **✅ UI COMPLETA (B0+B1, v1.0.15):** infraestructura i18n (`LocaleHelper`+`attachBaseContext`), selector Automático/Español/English en Ajustes, y TODA la UI migrada a `strings.xml`/`values-en` (nav, Biblioteca, Historial, Reproductor, Modo Auto, 8 diálogos, Login, Subir PDF, Ajustes). **Pendiente B2/B3 del plan:** voces de narración EN (según idioma del libro) + landing/tienda bilingües. | Alta |
| B7 | **Marcos 3D reales (PREMIUM futuro)** | Modelos 3D de verdad con profundidad y rotación usando un motor 3D (OpenGL / SceneView / Filament). Es un proyecto grande y pesado aparte. Se ofrecería como **opción premium** para usuarios visuales. | Baja (premium) |
| B11 | **Filtro de idioma/país en el selector de voces** | Con 63 voces (42 ES + 21 EN), añadir chips ES/EN (o por país) o un buscador en `VoiceSelectorDialog`. Opcional: sugerir voz EN automáticamente cuando la app está en inglés. **← PRÓXIMO PUSH** | Media |
| B8 | **Fuentes artísticas por género** | **✅ HECHO.** 12 tipografías `.ttf` reales incluidas en `res/font/` (Google Fonts vía Fontsource, convertidas a TTF): Creepster (terror), Uncial Antiqua/MedievalSharp (fantasía/medieval), Dancing Script (romance), Special Elite (noir), Orbitron (cósmico), Playfair/Cormorant/EB Garamond (poesía/espiritual/clásico), JetBrains Mono (científico), Comic Neue (comedia), Oswald (guerra). `resolveGenreFont` las usa por género, con fallback a familia integrada. Toggle Género/Serif/Sans en la barra. | Media |
